...
value nekoldap_get_entries(value neko_ldap, value neko_ldap_result)
{
	...
	LDAPMessage *ldap_result = (LDAPMessage *)val_to_kind(neko_ldap_result, k_ldap_message),
		*ldap_result_entry;
	LDAP *ldap = (LDAP *)val_to_kind(neko_ldap, k_ldap_pointer);
	...
	while (ldap_result_entry != NULL)
	{
		...
		while (attribute != NULL)
		{
			...
			for (i = 0; i < num_values; i++) {
				val_array_value(neko_attribute)[i] = alloc_string(ldap_value[i]->bv_val);
			}
